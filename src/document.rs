...
impl Document<Unchecked> {
    pub fn new(vertices: Vec<Vertex>, face_refs: Vec<FaceRef>, edge_count: Option<u64>) -> Self {
        Self {
            vertices,
            face_refs,
            edge_count,
    ...
    pub fn validate(self) -> Result<Document<Checked>, ValidationError> {
        let mut vertex_ref_iter = self.face_refs.iter().flat_map(|fr| &fr.vertex_refs);

        if let Some(invalid_vertex_index) = vertex_ref_iter.find(|vr| vr.0 >= self.vertices.len()) {
